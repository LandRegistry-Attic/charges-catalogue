...
class TestGovukAssets (unittest.TestCase):
    ...
    def test_image(self,client):
        res = client.get('/assets/images/favicon.ico')
    ...
    def test_js(self,client):
        res = client.get('/assets/javascripts/govuk-template.js')
        self.assertEqual(res.status_code, 200)
        self.assertTrue("application/javascript" in res.headers['Content-Type'])
